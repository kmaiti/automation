...
if [ "$TYPE" == "san" ]; then
	...
	echo -n "Enter igroup name to map: "                                   #Take igroup name
	read  MYIGROUP
	echo
...
if [[ -z "$MYIGROUP" ]];then
	echo -e "MYIGROUP Variables are not set properly :$red [FAILED]${nc}" >>$LOG
	usage && exit 0
else
        echo -e "MYIGROUP Variables are validated : $green [OK] $nc" >> $LOG
fi
...
MSG
	...
	check_stat=0
	LOGIN_TO_MACHINE=$(echo "${SSHPASS} -p ${password} ssh -o StrictHostKeyChecking=no ${user}@${host_to_login}.${DOM}" )
#ora_pmon_atpetinf is hard coded for att-pet-oracle-3d host
	COM1="sudo ps -ef|egrep -i 'pmon'|grep -vi 'ora_pmon_atpetinf'|grep -v grep|wc -l"
	COM2="sudo ps -ef|egrep -i 'pmon'|grep -vi 'ora_pmon_atpetinf'|grep -v grep"
...
MSG
	user=$1; password=$2; host_to_login=$3;
        LOGIN_TO_MACHINE=$(echo "${SSHPASS} -p ${password} ssh -o StrictHostKeyChecking=no ${user}@${host_to_login}.${DOM}" )
	COM="sudo /sbin/vxdg list|egrep -i '$FLEX_PREFIX'|egrep -i '$APP'|egrep -vi 'arch|name'"
        get_dg=$($LOGIN_TO_MACHINE  $COM)
        DG_LIST=$(echo "$get_dg"|awk '{print $1}')      #setting value to a global variable
	
	if [[ -z "$DG_LIST" ]];then
        echo -e "DG_LIST Variable is not set properly in function get_dg_list :$red [FAILED]${nc}" >>$LOG
        exit 0
else
        echo -e "DG_LIST variable is set & validated : $green [OK] $nc" >> $LOG
fi
...
MSG
	...
	while read dg 
	  do    
		...
		original_dg=$(echo $dg|awk -F"$FLEX_PREFIX" 'BEGIN {IGNORECASE = 1} {print $2}'|sed 's/^_//');
...
1. Check if TYPE is san
	1. If yes, then 
		call :  1. validate_sshpass > validate_login >  validate_sanlun > validate_running_db
			...
			5. export_dg
			6. destroy_flex_volumes
			7. create_flexclone
			8. map_flex_volume_luns
			9. import_dg
			10. report/verification : validate_existing_flex_vols_asLUN
